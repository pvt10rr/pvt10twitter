...
namespace Rapid_Reporter
{
    ...
    public partial class SMWidget : Window
    {
        ...
        private void Twitter_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (currentStage == sessionStartingStage.twitterAccount || Session.twitterPIN != null)
                {
                    twitter = true;
            ...
            else if (e.Key == Key.Enter)
            {
                ...
                if (e.Key == Key.Enter && NoteContent.Text.Trim().Length != 0)
                {
                    ...
                    if (currentStage == sessionStartingStage.twitterAccount)
                    {
                        ...
                        Session.twitterPIN = currentSession.twitterAccount;
                        if (Session.twitterPIN.Length != 7)
                        {
                            MessageBox.Show("Invalid pin. Try again later.", "Error");
                            return;
                        }
                        Session.twitterOAuth();
                        currentTwitterAccount.Text = " Twitter Account: " + Session.ScreenName;
                        StateMove(sessionStartingStage.tester);
                    }
