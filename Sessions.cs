...
namespace Rapid_Reporter
{
    class Session
    {
        ...
        public static void twitterLogin()
        {
            ...
            {
                //MessageBox.Show("Could not start your browser.", "Error");
            }

            //MessageBox.Show("After you have granted the application access, you will be given a 7 digit pin code.");
        }

        public static void twitterOAuth()
        {
            OAuthTokenResponse accessToken = OAuthUtility.GetAccessToken("WxOSthlIyUDQzWiGzY7F5Q", "TD3TbCa9BshneTMJTgdTmm57wzfQrQdXa8Ex2Sd7BkM", requestToken, twitterPIN);
        ...
        public void StartSession()
        {
            ...
            UpdateNotes("Hash Code", hashCode);
            if (ScreenName != null)
            {
                UpdateNotes("Twitter Account", ScreenName);
            }
            UpdateNotes("Session Reporter", tester);
            UpdateNotes("Session Charter", charter);
            UpdateNotes("Twitter Link", "https://twitter.com/#!/search?q=%23" + hashCode);
        }
        ...
        public void UpdateNotes(int type, string note, string screenshot, string RTFNote)
        {
            ...
            if (SMWidget.twitter)
            {
                string twitterPost = "[Reporter: " + tester + ", Charter: " + charter + "] " + note + " #" + noteTypes[type] + " #" + hashCode;
                string twitterNote;
                ...
                TwitterResponse<TwitterStatus> tweetResponse = TwitterStatus.Update(tokens, twitterPost);
            }
